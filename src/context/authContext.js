...
const DjangoAuthProvider = ({children}) => {
    ...
    const register = async ({is_service_provider, userName, password, email, confirmPassword}) => {
        try {
            const res = await axios.post(`${process.env.NEXT_PUBLIC_SERVICEFINDER_API_URL}/api/user/register/`, 
            {display_name:userName, username:userName, email:email, password:password, confirm_password:confirmPassword, is_service_provider:is_service_provider});
